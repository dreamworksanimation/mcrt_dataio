...
public:
    ...
    /// @param callBackRenderCompleteProcedure call-back function if command is "RenderComplete".
    /// @return Return callBack result status or false is it's not a McrtCommand or error happend.
    ///
    /// @detail
    /// So far we only have one McrtControl command. This is why we only have one call-back argument.
    /// We will have more than one call-back function definitions when we add more McrtCommands in the future.
    ...
    /// callBackRenderCompleteProcedure's 1st argument is syncId which you should compare with computation's
    ...
    bool run(const std::string &cmdLine,
             std::function<bool(uint32_t /*syncId*/)> callBackRenderCompleteProcedure);
